---
title : "Anthropic의 장시간 바이브 코딩을 위한 하네스"
date : 2026-04-08 21:40:00 +09:00
categories : [Claude, Harness]
tags : [Harness]
---

## 1. Background — 세션 경계라는 병목
...
이 글에서는 그 병목을 해소하는 방법(**harness**)이 무엇인지 알아보고, 다른 방법들과는 어떻게 다른지를 정리해본다.

## 2. Summary — Anthropic이 제안하는 harness의 골격

[원문](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)은 크게 세 가지 가이드라인으로 요약된다. 두 개의 모드, 네 개의 artifact, 그리고 모든 코딩 세션이 따라야 하는 7단계 workflow.

**두 개의 모드.** 같은 모델과 같은 도구를 쓰지만, 프롬프트만 바꿔서 에이전트를 두 가지 역할로 운영한다. *Initializer 모드*는 프로젝트의 뼈대를 깐다. 요구사항을 feature 단위로 쪼개고, 테스트 전략을 정하고, 작업 목록 파일을 만든다. 첫 번째 세션에 한 번만 돈다. *Coding 모드*는 그 뼈대 위에서 실제 기능을 하나씩 구현한다. 세션마다 반복해서 돈다.

**네 개의 artifact.** Harness가 유지되는 것은 모델의 기억력 덕분이 아니라, 세션 밖에 쌓이는 네 개의 파일 덕분이다.

- `feature_list.json` — 할 일 목록. 반드시 JSON이고, 반드시 머신-리더블이다. 원문은 이 규칙을 강하게 못박는데, 이유가 흥미롭다. Markdown 체크리스트는 사람이 보기엔 예쁘지만, 에이전트가 "이 항목이 끝났는지" 판단할 때마다 자연어를 다시 해석해야 한다. JSON의 `passes: true/false` 같은 boolean 필드는 해석 비용이 0에 가깝다. 작은 차이 같지만, 세션이 길어질수록 이 차이가 context window 효율을 바꾼다.
- `init.sh` — 세션을 시작할 때마다 실행되는 스크립트. 저장소 상태를 점검하고, 빌드가 되는지 확인하고, 테스트가 돌아가는지 확인한다. "이전 세션이 멀쩡한 상태로 끝났는가?"를 사람 대신 묻는 관문이다.
- `claude-progress.txt` — 세션 간 인수인계 노트. 이번 세션이 무엇을 했고, 무엇이 깨졌고, 다음 세션이 어디서부터 이어야 하는지가 적힌다. 다음 세션의 첫 작업은 항상 이 파일을 읽는 것이다.
- **Git 저장소** — 위 세 파일이 흩어지지 않게 묶어두는 바인더. 모든 세션의 끝에서 commit이 강제된다. commit은 곧 체크포인트다.

**7단계 workflow.** 모든 coding 세션은 동일한 순서를 지킨다 — Orient(progress 읽기), Verify(init.sh 돌리기), Select(다음 feature 고르기), Implement, Test end-to-end, Commit/Document, Leave merge-ready. 순서도 중요하지만 그보다 중요한 두 가지 규칙이 여기에 섞여 있다. 첫째, 버그가 남아 있으면 새 feature를 시작하지 않는다. 둘째, 한 세션에 한 feature만 닫는다. 이 두 규칙이 나머지 구조를 작동하게 만든다.
...
**`feature_list.json`이 JSON인 이유.** Section 2에서는 이 규칙을 "해석 비용이 싸다 / context budget을 아낀다"로 정리했다. 그런데 더 본질적인 동기는 효율이 아니라 *신뢰*에 있다. Markdown 체크리스트는 사람이 보기에도 예쁘지만, 무엇보다 *에이전트가 보기에도 편집하기 좋은 포맷*이다. 한 줄 지우고 다른 한 줄을 끼워넣는 것이 자연어 안에서 너무 매끄럽게 일어난다. 반면 `passes: true/false`로 못박힌 JSON 항목을 에이전트가 슬쩍 고쳐쓰는 것은 어색하다. 스키마를 깨야 하기 때문이다. 즉 JSON 포맷은 "사람이 정한 todo list를 에이전트가 조용히 다시 쓰는 것"을 막기 위한 가드레일이다. 효율은 부산물이고, 본질은 권한 분리다. 누가 spec을 바꿀 수 있는가에 대한 답을 파일 포맷 차원에서 못박아둔 것이다.

**두 모드를 왜 굳이 나누는가.** Initializer와 coding 모드가 같은 모델에 다른 프롬프트라면, 그냥 하나의 긴 프롬프트에 두 단계를 다 적어두면 안 되나? 처음엔 단순히 "역할 분리가 깔끔해서"라고 생각했는데, 다시 읽으니 더 분명한 이득이 있다. Coding 모드 프롬프트에는 *계획을 다시 짠다는 어휘 자체가 없다*. "feature를 재정의한다"거나 "전략을 수정한다"는 말이 프롬프트 어디에도 등장하지 않으니, 에이전트가 그 방향으로 흘러갈 수 있는 통로가 처음부터 막혀 있다. 금지 사항을 길게 나열하는 대신, *어휘를 빼버리는 것*이 가드레일이 된다. 가장 저렴한 가드레일은 "하지 마"가 아니라 "그 단어를 모름"이다. 두 모드 분리는 이 메커니즘을 활용하기 위한 구조적 트릭에 가깝다.
...
먼저 `feature_list.json`을 만들어야 한다. Initializer 모드가 한 번 돌고 나면 이런 모양이 나올 것이다.
...
세 개로 쪼갠 이유는 단순하다. 하나의 세션에 feature 하나씩만 닫는다는 규칙이 있으니, 하나의 feature 안에 "엔티티도 만들고 엔드포인트도 만들고 검증도 추가한다"를 다 우겨넣으면 그 세션은 절대 끝나지 않는다. 거꾸로 너무 잘게 쪼개면 한 세션이 5분 만에 끝나면서 commit/document 비용이 작업 비용보다 커진다. "세션 1개 분량 = entry 1개"라는 규칙으로 이 파일을 작성했다.
...
7단계 workflow도 비슷한 정도로 우리에게 익숙하다. Orient(progress 읽기) = 어제 작성한 PR 설명과 코멘트 다시 읽기. Verify(init.sh) = 로컬에서 `bootRun` 한 번 띄워보기. Implement → Test E2E → Commit → Leave merge-ready = 한 PR을 머지 가능한 상태로 떠나는 흐름 그 자체. "세션 하나에 feature 하나만 닫는다"는 규칙은 "한 PR에 한 가지 일만 한다"는 우리가 이미 지키려고 하는 원칙이고, "버그가 남아 있으면 새 feature를 시작하지 않는다"는 "CI red 상태에서 새 브랜치를 따지 않는다"와 닮아있다. 결국 이 harness가 에이전트에게 부과하는 룰은 사람에게 부과되는 것과 거의 같다. 다른 점은 하나뿐이다. 사람은 규칙을 어기게 되면 본인이 부끄러워서 다음번엔 지키려 하지만, 에이전트는 부끄러움이 없으므로 파일과 스크립트로 강제해야 한다.
...
## 5. 관련 오픈소스 — 같은 문제를 다르게 푸는 세 갈래

### 5.1 같은 문제, 세 개의 다른 시점

원문이 제안하는 harness가 풀려는 문제 — *에이전트가 spec을 잃어버리거나 멋대로 바꾸는 것* — 는 분명히 한 가지인데, 이걸 푸는 방식은 한 가지가 아니다. 이 문제를 어디서 잡느냐에 따라 도구가 갈린다.

에이전트가 코드를 만들어내는 흐름은 대략 이렇게 생겼다. 사람의 의도가 spec으로 굳고, spec을 가지고 코드가 생성되고, 그 코드가 실행되고 검증된다. 이 흐름의 *어느 지점에 손을 대느냐*가 도구의 성격을 결정한다.
...
다만, 한 가지 사실을 짚고 넘어가자. 세 프로젝트는 모두 흐름의 여러 지점에 동시에 손을 대고 있다. OMC도 `/deep-interview` 같은 명령으로 코드 생성 전에 모호성을 노출시키고, Q00도 코드 생성 *후* 단계에 다단계 검증을 둔다. 위에서 말한 "어느 시점"이라는 분류는 각 프로젝트가 *가장 무게를 두는 곳*을 기준으로 한 단순화다. 본론은 그 단순화 위에서 진행한다.

### 5.2 oh-my-claudecode — 7-step workflow를 런타임에 박아넣기

가장 먼저 볼 도구는 **oh-my-claudecode**(OMC)다. 한국 개발자 Yeachan Heo가 만들었고, instructkr 커뮤니티를 중심으로 활발하게 개발되고 있는 Claude Code 플러그인이다.

5.0의 분류로 말하면, OMC는 spec이 만들어지고 코드가 실행되는 동안에 손을 대는 도구다. 원문이 사람의 손에 남겨둔 그 자리를 정확히 노린다.
...
여기서 흥미로운 건 ralph mode가 *완전히 새로운 무언가*가 아니라는 점이다. Section 2에서 정리한 7-step workflow — Orient → Verify → Select → Implement → Test → Commit → Leave merge-ready — 를 다시 떠올려보자. 원문에서는 이 7단계가 *사람이 한 세션마다 손으로 굴리는* 절차였다. 사람이 progress를 읽고, 사람이 init.sh를 돌리고, 사람이 다음 feature를 골랐다. OMC 측은 이렇게 묘사하지 않지만, 두 패턴을 나란히 놓고 보면 ralph mode는 이 7단계를 사람의 손에서 떼어내 런타임 안으로 옮긴 것이다. 7단계의 *내용*은 거의 그대로지만, 그것을 굴리는 *주체*가 바뀐 것이다.
...
### 5.3 oh-my-codex — 같은 역할, 다른 런타임

**oh-my-codex**(OmX)는 OMC를 만든 같은 저자(Yeachan Heo)가 OpenAI Codex CLI 위에 옮겨놓은 자매 프로젝트다. 5.0의 분류로는 OMC와 똑같이 *코드가 실행되는 동안에 손을 대는* 도구이고, 역할도 같다. 다른 점은 그 기능을 어떻게 노출하는가이다. OMC가 Claude Code 플러그인 런타임 안쪽에 기능을 박아넣었다면, OmX는 `omx team queue`, `omx team claim`, `omx team complete` 같은 CLI 명령으로 같은 task lifecycle을 바깥으로 끄집어낸다. 워커는 tmux pane 위에서 돈다. 같은 패턴, 다른 표면이다.
...
### 5.4 Ouroboros — spec이 굳기 전에 잠근다

마지막으로 볼 도구는 **Q00/ouroboros**다. 5.0의 분류로 말하면, spec이 굳기 *직전*에 손을 대는 도구다. 사람의 모호한 요구사항이 spec으로 변환되는 그 순간을 노린다.
...
이 잠금 장치는 원문의 그것과 닮아있으면서도 다르다. 원문에서는 JSON 스키마가 "이미 만들어진 spec을 에이전트가 슬쩍 다시 쓰지 못하게" 잠근다. Ouroboros에서는 명확도 게이트가 "애매한 의도가 spec으로 굳어버리는 것 자체를" 잠근다. 둘 다 권한 분리의 도구이지만, 한쪽은 spec이 만들어진 다음을 지키고, 다른 한쪽은 spec이 만들어지는 그 순간을 지킨다. 이 차이는 곧 5.0에서 말한 "어느 시점에 손을 대느냐"의 차이다.

Ouroboros이 그 후에 두는 3단계 검증(Mechanical → Semantic → Multi-Model Consensus)도 물론 있지만, 이 도구의 무게중심은 그 뒤쪽이 아니라 앞쪽이다. 코드 한 줄이 생기기 전에 "이건 아직 시작할 수 없다"고 멈출 수 있다는 사실, 그게 ouroboros이 다른 두 진영과 갈라지는 자리다.

### 5.5 상류부터 하류까지 한 바퀴 돈 뒤에는
...
여기까지 정리한 내용의 출발점이 된 글은 Anthropic Engineering 블로그에 올라온 [**Effective harnesses for long-running agents**](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)다. Justin Young이 쓴 글로, Claude Agent SDK가 여러 context window를 가로질러 작업할 수 있게 만든 initializer/coding 두 모드 패턴을 처음으로 정리해 공개했다. 이후 Anthropic은 같은 문제에 multi-agent 구조로 답한 후속 글도 냈는데, 이 글은 그 흐름의 *첫 번째* 글 — 단일 에이전트와 외부 artifact만으로 어디까지 갈 수 있는지를 보여주는 출발점 — 에 해당한다. 후속 글들은 이 시리즈의 2편과 3편에서 차례로 다룰 예정이다. 
