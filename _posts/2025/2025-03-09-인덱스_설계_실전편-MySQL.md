...
만약 조회 작업에 인덱스를 사용할 수 없다면, 모든 레코드(테이블 전체)를 다 불러와서 학번을 찾아야 한다. 그런데 모든 레코드를 읽어들이는 것은  데이터베이스에서 **가장 비용이 많이 드는** 작업인 **디스크 I/O** 이 늘어나는 것이기 때문에 결과적으로 아주 비싼 작업을 수행하게 되버린다.

배송 기사(데이터베이스)가 트럭으로 운전하는 작업(조회 작업)을 5톤 트럭으로 1번 왕복하는 것이 인덱스를 사용하는 것이라면, 1톤 트럭으로 5번 왕복하며 화물을 옮기는 것은 인덱스를 사용하지 못하는 것과 흡사하다. 하지만 단지 흡사할 뿐이니까, 동일시는 하지말자
