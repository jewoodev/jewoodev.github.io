...
**MySQL의 경우:**
- 3개 테이블에서 데이터를 읽어야 함
- JOIN 연산으로 인한 CPU 오버헤드 발생
- 인덱스 스캔이 여러 테이블에 걸쳐 발생
- 네트워크 I/O가 여러 번 발생할 가능성
...
하나의 Policy 정보를 조회할 때는 **단일 위치에서 순차적으로** 모든 데이터를 읽는다.
...
### 성능 수치로 보는 차이
실제 벤치마크에서 이런 차이가 어떻게 나타나는지 살펴보자:
...
대용량 데이터 환경에서 이러한 데이터 지역성 차이는 더욱 극명해진다. 수백만 건의 정책 데이터가 있을 때:
...
2. **스토리지 엔진**: WiredTiger의 Lock-Free 아키텍처와 혁신적인 메모리 관리
3. **고급 인덱싱**: 다양한 쿼리 패턴에 최적화된 특화 인덱스 지원

이러한 기술적 혁신들이 결합되어 MongoDB는 전통적인 RDBMS 대비 향상된 처리량과 효과적인 워크로드 분산을 달성할 수 있다.
다만 MongoDB의 유연성이 스키마 설계의 중요성을 간과해도 된다는 의미는 아니다. 프로젝트 초기부터 스키마 설계 모범 사례를 적용하는 것이 향후 리팩토링 노력을 절약하는 핵심이다.
